...
def on_upload(source, target, env):
    ...
    with open(firmware_path, 'rb') as firmware:
        ...
        encoder=""
        if(firmware.name.split("\\")[-1] == "firmware.bin"):
            encoder = MultipartEncoder(fields={
                'MD5': md5, 
                'firmware': ("firmware", firmware, 'application/octet-stream')}
            )
        elif(firmware.name.split("\\")[-1] == "littlefs.bin"):
            encoder = MultipartEncoder(fields={
                'MD5': md5, 
                'filesystem': ("filesystem", firmware, 'application/octet-stream')}
            )
        ...
        response = requests.post(upload_url+"/update", data=monitor, headers={'Content-Type': monitor.content_type})
        bar.close()
        time.sleep(0.1);
